...
def _maintenance_resp(leaf_biomass, stem_biomass, root_biomass, 
    ...
    resp_leaf = leaf_biomass * 0.003 * temp_factor
    resp_stem = stem_biomass * 0.0015 * temp_factor
    resp_root = root_biomass * 0.001 * temp_factor
    resp_fruit = fruit_biomass * 0.001 * temp_factor
...
class CropgroStrawberry:
    def __init__(self, latitude, planting_date, soil_properties, cultivar_params):
        ...
        self.phenology_stages = {
            ...
            'JUVENILE': 250, 
            'VEGETATIVE': 500,
            'FLORAL_INDUCTION': 800,
            'FLOWERING': 1200,
            'FRUIT_SET': 1500,
            'FRUIT_DEVELOPMENT': 1800,
            'FRUIT_MATURITY': 2200,
            'SENESCENCE': 3000
        }
        self.results = []
    ...
    def plot_results(self):
        ...
        axs[0, 1].set_ylabel('LAI (m^2/m^2)')
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("CROPGRO-Strawberry Model Simulation Results")
    print("=" * 60)
    print("Simulation days: %d" % len(results))
    print("Planting date: 2023-05-01")
    print("End date: 2023-10-31")
    print("-" * 60)
    print("Final total biomass: %.2f g/plant" % results['biomass'].iloc[-1])
    print("Final fruit biomass: %.2f g/plant" % results['fruit_biomass'].iloc[-1])
    print("Final leaf biomass: %.2f g/plant" % results['leaf_biomass'].iloc[-1])
    print("Final stem biomass: %.2f g/plant" % results['stem_biomass'].iloc[-1])
    print("Final root biomass: %.2f g/plant" % results['root_biomass'].iloc[-1])
    print("Final leaf area index: %.2f m^2/m^2" % results['leaf_area_index'].iloc[-1])
    print("Final fruit number: %.2f fruits/plant" % results['fruit_number'].iloc[-1])
    print("Final crown number: %.2f crowns/plant" % results['crown_number'].iloc[-1])
    print("Final runner number: %.2f runners/plant" % results['runner_number'].iloc[-1])
    print("Final phenological stage: %s" % results['stage'].iloc[-1])
    print("-" * 60)
    
    fig.savefig('d:\\综合实训\\CN-strawberryDSSAT-main\\simulation_results.png', dpi=150)
